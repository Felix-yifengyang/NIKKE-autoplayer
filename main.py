...
class TaskThread(QtCore.QThread):  # 每个 TaskThread 对象都代表一个独立的线程
    ...
    def run(self):
        if self.task_number == 8:  # Q键连点
            auto_task.continuous_click()
        else:
            self.start_time = time.time()  # 记录任务开始时间
            if self.task_number == 1:
                auto_task.gain_rewards(arena_shop_task)   # 收米
            elif self.task_number == 2:
                while True:
                    auto_task.recruit()  # 招募
                    time.sleep(1)
            elif self.task_number == 3:
                auto_task.simulation_room(overclocking)  # 模拟室
            elif self.task_number == 4:
                auto_task.auto_consult()  # 咨询
            elif self.task_number == 5:
                auto_task.auto_arena()  # 竞技场
            elif self.task_number == 6:
                auto_task.union_battle()  # 联盟战
            elif self.task_number == 7:
                while True:
                    auto_task.auto_all(auto_task_list, arena_shop_task, overclocking)  # 一键摆烂
                    time.sleep(1)
